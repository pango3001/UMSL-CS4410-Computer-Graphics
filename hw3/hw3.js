...
// DATE: Oct 8 2020
// FILE: hw3.js
// DESCRIPTION: Modify wave.js (m3.zip) so that the sine wave
//     would be symmetric across y-axis (left half and right half
//     are like twins but moving in the opposite directions)
...
// left side
  for (let i = -half; i <= half; i++) { // draw all stars    
    ...
    modelMatrix.scale(0, sy, 1); // scaling up and down
...
// center and right side
  for (let i = -half; i <= half; i++) { // draw all stars    
	...
	let sy = (Math.sin(-iTime * 2 + dist)); // opposite directional flow of left side
    ...
    modelMatrix.setIdentity();  // Set identity matrix
    modelMatrix.translate(0, h * 0.5 * sy - 0.6, 0); // make all bars align horizontally
    if(x >= 0){ // includes 0 to include center and the right side
    modelMatrix.scale(1, sy, 1); // scaling up and down
    }else 
    modelMatrix.scale(0, sy, 1); // scaling up and down
