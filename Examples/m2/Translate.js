...
let d = 0.5; // displacement
let speed = 0.5; // speed of animation
let sign = 1.0; // rightward move
...
function draw() {
  modelMatrix.setTranslate(d, d, 0); 
...
function animate() {
  ...
  prev = now;
  d += sign * deltaTime * speed;
  if (Math.abs(d) > 0.5) { d = sign * 0.5; sign *= -1; }
}
